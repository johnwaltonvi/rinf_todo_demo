...
pub async fn create_actors() -> messages::prelude::Address<FirstActor> {
    ...
    let second_actor = SecondActor::new(first_addr.clone());
    spawn(second_context.run(second_actor));

    // Return the address of the first actor
    first_addr
}
